...
class _HabitsPageState extends State<HabitsPage> {
      ...
      builder: (ctx) => Column(
        ...
        children: [
          ListTile(
            ...
            subtitle: Text(habit['frequency'], style: const TextStyle(color: Colors.redAccent)),
          ...
          ListTile(
            ...
            onTap: () {
              Navigator.push(context, MaterialPageRoute(builder: (context)=>ArchivedHabitsPage()));
            },
          ),
          ListTile(
  ...
  Widget buildHabitCard(Map habit, String habitId) {
    ...
    final doneCount = last7.where((done) => done).length;
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(habit['habitName'], style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                Container(
                  padding: const EdgeInsets.symmetric(vertical: 4, horizontal: 8),
                  decoration: BoxDecoration(color: Colors.red[900], borderRadius: BorderRadius.circular(8)),
                  child: Text(habit['frequency'], style: const TextStyle(color: Colors.white70, fontSize: 12)),
                ),
                Row(
                  children: [
                    IconButton(icon: const Icon(Icons.more_vert, color: Colors.white70), onPressed: () => _showHabitOptions(habit, habitId)),
                  ],
                ),
              ],
            ),
            ...
            SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  final val = last7[i];
                  Color bg;
                  if (val) bg = Colors.green;
                  else if (!val && day.isBefore(DateTime(today.year, today.month, today.day))) bg = Colors.red;
                  else if (DateUtils.dateOnly(day) == DateUtils.dateOnly(today)) bg = Colors.yellow;
                  else bg = Colors.grey;
                  return Container(
                    width: 48,
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][day.weekday % 7], style: const TextStyle(color: Colors.white70, fontSize: 10)),
                        Text('${day.day}', style: const TextStyle(color: Colors.white, fontSize: 16)),
                      ],
                    ),
                  );
                },
            ...
            Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    IconButton(icon: const Icon(Icons.calendar_today_outlined, color: Colors.white70), onPressed: () => _showHabitOptions(habit, habitId)),
                  ],
                )
              ],
            ),
  ...
  Widget _noHabitsView() {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Text("No active habits found", style: TextStyle(color: Colors.white70)),
          const SizedBox(height: 12),
          ElevatedButton.icon(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const ArchivedHabitsPage()),
              );
            },
            icon: const Icon(Icons.unarchive, color: Colors.white),
            label: const Text("View Archived Habits", style: TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        ...
        builder: (ctx, snap) {
          ...
          if (entries.isEmpty) {
            return _noHabitsView(); // Reuse same widget
          }
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ActivityTypePage())),
